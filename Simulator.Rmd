...
P.long.df <- P.long.complete.df %>% 
  ...
  distinct() %>% 
  mutate(supertile.id = create_supertile_index(., elements = c("tile.id.chr", "cell.chr", "pij"))) %>% 
...
In the Estimation part we configure all necessary parameters for the defined estimators. At the end, all final estimates are bounded together in one dataset on the tile level. Currently 5 estimators are implemented (which represent to the best of my knowledge all currently relevant ones).

## Estimation: Setup

Here we specify the connection probability models. In this case P = P\*. Furthermore, we define the c-vector, which contains the number of mobile phones an antenna has connected to, as well as the prior vector, which is defined as a constant for each tile (here 1). For the following estimator functions the parameters are assumed to be in a `datatable` format or sparse Matrix via `sparseMatrix`.
...
The third option is also on the antenna level and uses the barycenter (location with the strongest signal dominance value of the antenna specific coverage area) of each antenna as seed location for the computation of Voronoi regions.
...
The last part of this notebook follows through with evaluating the different estimators based on the following indicators: 1d density, 2d density, Spatial density (i.e., geographical maps), KWD, and Convergence based on KWD. Further evaluation metrics or different version of the listed ones will be added at a later stage.

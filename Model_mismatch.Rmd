...
### prior vector versions
a.tile.helper.informative <- area$area.df %>% 
  dplyr::select(contains("tile.id.num"), prior.informative.3)

a.tile.vec.informative <- a.tile.helper.informative %>% 
  deframe()

a.tile.dt.informative <- a.tile.helper.informative %>% 
  dplyr::select(j = contains("tile.id.num"), u = prior.informative.3) %>% 
  as.data.table()


a.tile.helper.uninformative <- area$area.df %>% 
  dplyr::select(contains("tile.id.num"), prior.uninformative)

a.tile.vec.uninformative <- a.tile.helper.uninformative %>% 
  deframe()

a.tile.dt.uninformative <- a.tile.helper.uninformative %>% 
  dplyr::select(j = contains("tile.id.num"), u = prior.uninformative) %>% 
  as.data.table()
...
### Specifically for Voronoi
VOR.input.list <- list(
  cellplan.combined = flatten(cellplans.list) %>% 
    keep(str_detect(names(.), "cellplan.combined.df")) %>%
    set_names(names(cellplans.list)), 
  signal.strength.comb.dt = flatten(cellplans.list) %>% 
    keep(str_detect(names(.), "signal.strength.comb.dt")) %>% 
    set_names(names(cellplans.list)), 
  C.vec.df =  gen.model.objects$C.vec.df.list)


cellplan.combined <- cellplans.list$cellplan.3.layer$cellplan.combined.df
signal.strength.comb.dt <- cellplans.list$cellplan.3.layer$signal.strength.comb.dt
C.vec.df <- gen.model.objects$C.vec.df.list$cellplan.3.layer
```
...
VOR.tower.informative <- pmap(VOR.input.list, ~VOR_est(area = area, 
                                                       cellplan.combined = ..1, 
...
VOR.tower.est.informative <- VOR.tower.informative %>% 
...
VOR.offset.informative <- pmap(VOR.input.list, ~VOR_est(area = area, 
                                            cellplan.combined = ..1, 
                                            signal.strength.comb.dt = ..2, 
                                            C.vec.df = ..3, 
                                            prior.var = "prior.informative.3",
...
VOR.offset.est.informative <- VOR.offset.informative %>% 
  map(~mutate(.x$seed.voronoi.final, tile.id.chr = as.character(tile.id))) %>% 
...
VOR.barycenter.informative <- pmap(VOR.input.list, ~VOR_est(area = area, 
                                                cellplan.combined = ..1, 
                                                signal.strength.comb.dt = ..2, 
                                                C.vec.df = ..3, 
                                                prior.var = "prior.informative.3",
...
VOR.barycenter.est.informative <- VOR.barycenter.informative %>% 
  map(~mutate(.x$seed.voronoi.final, tile.id.chr = as.character(tile.id))) %>% 
...
final.estimates.sf <- area$area.sf %>% 
  left_join(VOR.tower.both, by = "tile.id.chr") %>%
  left_join(VOR.offset.both, by = "tile.id.chr") %>%
  left_join(VOR.barycenter.both, by = "tile.id.chr")
